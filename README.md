...
3. Заменяет 7 плейсхолдеров (`msElenaDolgova`, `/Users/elenadolgova/IWE` и др.)
...
1. Замените `msElenaDolgova` на ваш GitHub username во всех файлах
2. Замените `/Users/elenadolgova/IWE` на путь к рабочей директории (напр. `~/IWE`)
3. Замените `/Users/elenadolgova` на домашнюю директорию (значение `$HOME`)
4. Замените `-Users-elenadolgova-IWE` на путь через дефисы (напр. для `~/IWE` → `-Users-yourname-IWE`)
5. Замените `10` на час запуска стратега в UTC (напр. `4` для 7:00 MSK)
6. Замените `0` на описание времени (напр. `7:00 MSK`)
