...
- (опционально, если настроены агенты-сборщики QA) Прочитай последние QA-отчёты в `/Users/elenadolgova/IWE/<agent-workspace>/<scheduler>/<reports>/`: замечания (✏️) первые, urgent (high/critical) вторые, кластеры третьи. Кластеры (≥3 = **urgent** → WP-debt) и resolved помечает Session-Prep. Если агентов нет — пропустить шаг.
