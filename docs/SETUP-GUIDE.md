...
> **Как это работает:** При открытии сессии Claude определяет класс верификации задачи и рекомендует модель. Пользователь соглашается или меняет. Если согласованная модель дешевле текущей — Claude делегирует задачу sub-agent'у на этой модели (сессия не прерывается). Переключение на более дорогую модель — только вручную через `/model`.
>
