...
1. В Claude Code скажи: **«подключи мой Google Calendar»**
2. Claude покажет ссылку → открой в браузере → залогинься через свой Google-аккаунт → разреши доступ
3. Проверь: **«покажи мои события на сегодня»**

> **Если OAuth не работает** (ошибка авторизации): нужно создать свой GCP-проект. Инструкция: [Google Cloud Console](https://console.cloud.google.com) → новый проект → включить Google Calendar API → Auth Platform → consent screen (External, добавить свой email в test users) → Clients → Desktop app → Create. После этого повтори шаг 1.
