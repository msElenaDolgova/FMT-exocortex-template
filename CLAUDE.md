...
6. **Чеклист-верификация (Haiku R23):** Любой протокол с секцией «Чеклист» → после выполнения алгоритма запустить sub-agent Haiku в роли R23 Верификатор. Он проходит каждый пункт чеклиста: ✅/❌. По ❌ — исправить ДО показа результата пользователю. Причина: контекст исполнителя загрязнён (VR.SOTA.002 context isolation).
...
> **Скилл ≠ Роль ≠ Протокол.** Скилл (`.claude/skills/`) = точечное действие с входом/выходом. Роль = контекст на всю сессию (CLAUDE.md репо). Протокол = ритуал стадии ОРЗ (protocol-*.md). Метод (Pack) → скилл (FMT) → рабочий экземпляр ({{WORKSPACE_DIR}}/.claude/skills/). Skill Discovery: `protocol-work.md §2b`.
