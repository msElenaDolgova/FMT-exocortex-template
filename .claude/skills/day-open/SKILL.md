...
2b. **Предложения Note-Review (WeekPlan):** в WeekPlan найти секцию `## Предложения` (или `Предложения (Note-Review`). Если есть незакрытые `[ ]` → добавить в «Требует внимания»: «В WeekPlan N необработанных предложений Note-Review — утвердить или отклонить перед стратсессией.» Ссылку на строку WeekPlan приложить.
