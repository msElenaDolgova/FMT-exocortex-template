...
### 2. План на сегодня
**Приоритет входов:**
1. Carry-over из шага 1 (задел от вчера) — идут первыми
2. WeekPlan + Linear check
3. `day-rhythm-config.yaml → mandatory_daily_wps` — обязательные РП (если есть в конфиге)

Итого: **все carry-over РП из Day Close** + mandatory. Не обрезать carry-over до фиксированного числа — если Day Close указал 7 РП на завтра, все 7 попадают в план. **Слот 1 = саморазвитие.**
